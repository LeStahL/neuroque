...
void main_scene(in vec3 x, out vec2 sdf)
{
    ...
    // rot3(vec3(1.1,1.3,1.6), Re);
            ...
            dspline3(Rx*z,-.05*c.yyx-.05*vec3(cos(co-iTime), sin(co-iTime),0.),.05*c.yyy, .1*c.yyx+.05*vec3(cos(co-iTime), sin(co-iTime),0.), da);
            dspline3((Rx*z).yzx,-.05*c.yyx-.05*vec3(cos(co-iTime), sin(co-iTime),0.),.05*c.yyy, .1*c.yyx+.05*vec3(cos(co-iTime), sin(co-iTime),0.), das);
            da = min(da, das);
            dspline3((Rx*z).zxy,-.05*vec3(cos(co-iTime), sin(co-iTime),0.)-.05*c.yyx,.05*c.yyy, .1*c.yyx+.05*vec3(cos(co-iTime), sin(co-iTime),0.), das);
            da = min(da, das);
            da = abs(da)-.006+.07*length(Rx*z);
            
            

            add(sdf, vec2(abs(da)-.001, 3.+co/2./pi), sdf);
...
void illuminate(in vec3 x, in vec3 n, in vec3 dir, in vec3 l, inout vec3 col, in vec2 s)
{
    ...
    {
        col = mix(vec3(0.87,0.05,0.43), .3*c.xxx, clamp(s.y-3.,0.,1.));
        col = .2*col
            + .3*col*max(dot(l-x,n),0.)
